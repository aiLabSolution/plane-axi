...
export const COMMAND_METADATA = {
  ...
  "wi list": spec("List work items", "plane-axi wi list [flags]", commands.wiList, {
    ...
    assignee: flag("Filter by member UUID, name, or email"), limit: flag("Maximum rows (default: 50)", { default: "50" }),
  ...
  "wi create": spec("Create a work item", "plane-axi wi create --title <title> [flags]", commands.wiCreate, {
    project: projectFlag, title: flag("Work item title", { required: true }), body: flag("Plain-text body"), priority: flag("Priority enum"),
  ...
  "wi update": spec("Update a work item", "plane-axi wi update <ref> [flags]", commands.wiUpdate, {
    project: projectFlag, title: flag("New title"), body: flag("New plain-text body"), priority: flag("New priority"), state: flag("New state UUID or name")
  ...
  "wi search": spec("Search work items across the workspace", "plane-axi wi search <query> [--limit <n>|--all]", commands.wiSearch, {
    limit: flag("Maximum rows (default: 50)", { default: "50" }), all: flag("Return all matches", { boolean: true })
  ...
  "setup": spec("Install session hooks or the generated skill", "plane-axi setup [--app <app>] [--scope <scope>] [--skill]", commands.setup, {
    app: flag("claude|codex|opencode|all (default: all)", { default: "all", choices: ["claude", "codex", "opencode", "all"] }),
    scope: flag("project|user (default: project)", { default: "project", choices: ["project", "user"] }), skill: flag("Install the generated Agent Skill", { boolean: true })
...
function resolveCommand(args) {
  ...
  const key = GROUPS.has(args[0]) ? `${args[0]} ${args[1] || ""}`.trim() : args[0];
  const consumed = GROUPS.has(args[0]) ? 2 : 1;
  if (!COMMAND_METADATA[key]) {
...
export async function run(args, options = {}) {
  const { key, rest } = resolveCommand(args);
...
export const _internals = { parseFlags, resolveCommand, commandHelp, topHelp };
