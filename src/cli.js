...
export const COMMAND_METADATA = {
  ...
  "wi search": spec("Search work item titles in the selected project", "plane-axi wi search <query> [--project <ref>|--workspace] [--text] [--limit <n>|--all]", commands.wiSearch, {
    project: projectFlag, workspace: flag("Search every project in the workspace", { boolean: true }),
    text: flag("Match bodies as well as titles (scans the project)", { boolean: true }),
    limit: flag("Maximum rows", { default: "50" }), all: flag("Return all matches", { boolean: true })
  }, ["query..."], ["plane-axi wi search authentication bug", "plane-axi wi search onboarding --text", "plane-axi wi search onboarding --workspace"]),
  "comment list": spec("List comments; --all also lists activity entries after them", "plane-axi comment list <wi-ref> [--all]", commands.commentList, { project: projectFlag, all: flag("Append all activity entries after the comments", { boolean: true }) }, ["wi-ref"], ["plane-axi comment list LABS-42", "plane-axi comment list LABS-42 --all"]),
